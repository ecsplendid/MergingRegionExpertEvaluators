function [ model ] = genetic_optimizeparams_regions( ...
...
best_params = ga( optimization_function, ...
    length(genetic_getbounds_regions( 1 )), ... % num constraints
    [],[],[],[], ...
    genetic_getbounds_regions( 1 ), ...     % lower
    genetic_getbounds_regions( 2 ), ...     % upper
    [], ...
    genetic_getbounds_regions( -1 ), ...    % int constraints
    options );
